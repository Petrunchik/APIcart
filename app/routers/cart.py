...
async def _ensure_product_available(db: AsyncSession, product_id: int) -> None:
    """
    Проверка на существование товара с указанным ID.
    """
...
async def _get_cart_item(db: AsyncSession, user_id: int, product_id: int) -> CartItemModel | None:
    """
    Ищет товар в корзине текущего пользователя и возвращает объект CartItemModel | None.
    """
    result = await db.scalars(
...
async def update_quantity_in_cart(
    product_id: int,

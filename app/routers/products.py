...
async def get_product(product_id: int, db: Session = Depends(get_db)):
    ...
    """
    db_product = db.scalars(select(ProductModel).where(
...
async def get_products_by_category(category_id: int, db: Session = Depends(get_db)):
    ...
    db_products = db.scalars(select(ProductModel).where(
...
async def update_product(product_id: int, updated_product: ProductCreate, db: Session = Depends(get_db)):
    """
    ...
    if db_category is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Category not found")
    db.execute(update(ProductModel).where(
        ProductModel.id == product_id,
        ProductModel.is_active == True
